// Adapted from pocket-stack/pocket-youtube host/native-media.ts (MIT). See THIRD_PARTY.md.
...
export interface ResolvedStream { videoUrl:string; audioUrl:string; width:number; height:number; token?:string; hasAudio?:boolean }
...
export async function* nativeMedia(source: ResolvedStream, seconds: number, signal: AbortSignal): AsyncGenerator<MediaPacket> {
  ...
  const auth = source.token ? ["-headers", `X-Emby-Token: ${source.token}\r\n`] : [];
  const seek = seconds > 0 ? ["-ss", String(seconds)] : [];
  const options = { stdout: "pipe", stderr: "pipe", env: { ...process.env,  } } as const;
  ...
  const video = Bun.spawn([...common, ...network(source.videoUrl), ...seek, ...auth, "-i", source.videoUrl,
  ...
  const audio = Bun.spawn([...common, ...(source.hasAudio === false ? ["-f", "lavfi", "-i", "anullsrc=r=22050:cl=stereo"] : [...network(source.audioUrl), ...seek, ...auth, "-i", source.audioUrl]),
    "-vn", "-ac", "2", "-ar", String(MEDIA.sampleRate), "-f", "s16le", "pipe:1"], options);
  const stop = () => { video.kill(9); audio.kill(9); };
  ...
  async function* videoPackets(): AsyncGenerator<MediaPacket> {
    ...
    if (await video.exited || !frame) throw new Error("Video encoder failed");
  }
  async function* audioPackets(): AsyncGenerator<MediaPacket> {
    ...
    if (await audio.exited || !count) throw new Error("Audio encoder failed");
  }
  const v = videoPackets(), a = audioPackets();
  try {
    let [vp, ap] = await Promise.all([v.next(), a.next()]);
    while (!vp.done && !signal.aborted) {
