...
test("compiled direct client browses, plays, pauses, seeks, searches and restarts",async()=>{
 ...
 let keyboardAccept=true;
 let session=1,signedIn=false,opened=0,position=0,phase="idle";
 const rows=Array.from({length:10},(_,i)=>({id:`a${i}`,name:["Big Buck Bunny","Sintel","Tears of Steel","Cosmos - The shore of the cosmic ocean","Planet Earth - From pole to pole"][i%5],type:"Movie",folder:false,seconds:600,resume:12,played:false,year:2024}));
 ...
 if(c.t==="keyboard")value={accepted:keyboardAccept,value:c.field==="search"?"Cosmos":c.field==="server"?"http://new-server:8096":c.value};
 if(c.t==="art")value={handle:texture,width:256,height:128};if(c.t==="hello")value={direct:true,authenticated:signedIn,server:"http://jellyfin.local:8096",username:"Mii"};if(c.t==="list")value={items:rows.slice(c.offset,c.offset+5),total:10,offset:c.offset};if(c.t==="play"){position=c.seconds;value={source:{host:"127.0.0.1",port:1,token:"0".repeat(64)},seconds:600,position};}
 ...
 tap(70,60);expect(commands.at(-1).t).toBe("keyboard");expect(commands.at(-1).field).toBe("server");
 keyboardAccept=false;tap(70,60);expect(commands.at(-1).value).toBe("http://new-server:8096");
 tap(70,60);expect(commands.at(-1).value).toBe("http://new-server:8096");
 tap(70,145);expect(commands.at(-1).field).toBe("password");
 ...
 for(let i=0;i<5;i++)press(0x40);expect(commands.filter(c=>c.t==="list").at(-1).offset).toBe(1);
 step(40);expect(commands.some(c=>c.t==="art")).toBe(true);
 tap(60,62);tap(60,62);await capture("detail");press(0x2000);expect(opened).toBe(1);expect(position).toBe(12);await capture("player");
 ...
 press(0x1);await capture("playing-browse");press(0x1000);expect(commands.at(-1).t).toBe("keyboard");expect(commands.at(-1).field).toBe("search"); // OS cancellation is mocked.
 keyboardAccept=true;press(0x1000);expect(commands.filter(c=>c.t==="list").at(-1).query).toBe("Cosmos");
